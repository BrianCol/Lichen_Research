...
# Import functions, clear workspace
rm(list = ls())
useShinyalert()
...
shinyServer(function(input, output, session){
  ...
  vals_trich$Data<-readRDS("working.rds")
  ...
  observeEvent(input$go, {
    ...
    vals_trich$Data<-data.table(rbind(vals_trich$Data, new_row, fill=TRUE) )
    removeModal()
  })
  ...
  observeEvent(input$Updated_trich,{
    saveRDS(vals_trich$Data, "working.rds")
    shinyalert(title = "Saved!", type = "success")
  })
  
  # Delete selected rows
  # Warning messge for deleting
 ...
 # Download csv 
  output$Trich_csv<- downloadHandler(
    filename = function() {
      paste("working_DNA_reference_library", Sys.Date(), ".csv", sep="")
    },
